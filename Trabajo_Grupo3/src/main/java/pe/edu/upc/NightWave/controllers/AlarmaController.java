...
public class AlarmaController {
    ...
    public ResponseEntity<?> listar() {
        List<AlarmaDTO> lista = aS.list()
                .stream()
                .map(alarma -> modelMapper.map(alarma, AlarmaDTO.class))
                .collect(Collectors.toList());

        if (lista.isEmpty()) {
            return ResponseEntity.status(HttpStatus.OK).body("No existen alarmas registradas.");
        }
    ...
    public ResponseEntity<String> registrar(@RequestBody AlarmaDTO dto) {
        ...
        aS.insert(alarma);
        return ResponseEntity.status(HttpStatus.CREATED).body("Alarma registrada correctamente.");
    }

    // 📌 Modificar
    @PutMapping
    public ResponseEntity<String> modificar(@RequestBody AlarmaDTO dto) {
        Alarma existente = aS.listId(dto.getIdAlarma());
        if (existente == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body("No se puede modificar. No existe una alarma con el ID: " + dto.getIdAlarma());
        }
        Alarma alarma = modelMapper.map(dto, Alarma.class);
        aS.update(alarma);
        return ResponseEntity.ok("Alarma con ID " + dto.getIdAlarma() + " modificada correctamente.");
    }
