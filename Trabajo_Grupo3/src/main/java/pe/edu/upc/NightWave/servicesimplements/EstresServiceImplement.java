...
import pe.edu.upc.NightWave.repositories.IEstresRepository;
...
public class EstresServiceImplement implements IEstresService {
    ...
    @Override
    public void insert(Estres estres) { eR.save(estres); }

    @Override
    public List<Estres> list() {return eR.findAll();}

    @Override
    public void delete(int id) {eR.deleteById(id);}

    @Override
    public Estres listId(int id) {return eR.findById(id).orElse(null);}

    @Override
    public void update(Estres estres) { eR.save(estres);}
}
