    ...
    public ResponseEntity<String> modificar(@RequestBody EstresDTO dto) {
        ...
        if (existente == null) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body("No se puede modificar. No existe control parental con ID: " + dto.getIdEstres());
        }

        eS.update(es);
        return ResponseEntity.ok("Control parental con ID " + dto.getIdEstres() + " modificado correctamente.");
    }
